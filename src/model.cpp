...
public:
    ...
    LaneModel(int n, float sigma_trans, float sigma_obs);
    ~LaneModel();
    //double * prediction();
...
LaneModel::LaneModel(int n, float P1, float P2, float P3, float P4, float sigma_trans, float sigma_obs)
{
    ...
    for (int mean = 1; mean <= num_lane; mean++)
    {
        ...
        prob_bn* emission_uniform = (prob_bn*)malloc (num_lane * sizeof (prob_bn));
        ...
        emission_states[0] = GetStateNamed_bn(buf, laneT1);
        emission_states[1] = GetStateNamed_bn("Not_Working", sensorStateT1);
        SetNodeProbs_bn (detector, emission_states, emission_uniform);

        free(transition_pb);
        free(emission_pb);
        free(emission_uniform);
...
//LaneModel without RI and SENSOR STATE
LaneModel::LaneModel(int n, float sigma_trans, float sigma_obs)
{
    use_RI = false;
    use_sensor_state = false;
    num_lane = n;
    res = CloseNetica_bn (env, mesg);
    printf ("%s\n", mesg);
...
const float * LaneModel::update(float * ev_detector, float * ev_RI)
{
    ...
    if(use_sensor_state) {
        ...
        ROS_DEBUG_STREAM("NOT USING SENSOR STATE");
