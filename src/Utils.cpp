...
///
/// \brief Utils::lla2ecef
/// \param lat geodetic latitude (radians)
/// \param lon longitude (radians)
/// \param alt height above WGS84 ellipsoid (m)
...
///
/// \brief Utils::addNoiseAndCovToOdom
/// \param step
/// \param pos_err
/// \param or_err
/// \param lin_err
...
///
/// \brief Utils::getSpeedFrom2PoseStamped
/// \param pose_prec
/// \param pose_t
...
VectorXd Utils::addOffsetToVectorXd(const VectorXd& pose, double position_offset, double orientation_offset, double speed_offset)
{
    ...
    vec(3) += Utils::getNoise(orientation_offset);
    vec(3) = atan2(sin(vec(3)), cos(vec(3)));
    vec(4) += Utils::getNoise(orientation_offset);
    vec(4) = atan2(sin(vec(4)), cos(vec(4)));
    vec(5) += Utils::getNoise(orientation_offset);
    vec(5) = atan2(sin(vec(5)), cos(vec(5)));
...
///
/// \brief Utils::getCameraCenterAfterRotation
/// \param c
/// \param r
/// \return
///
...
double Utils::rad2deg (double alpha)
{
    return (alpha * 57.29578);    /// from PCL library
}
double Utils::deg2rad (double alpha)
{
    return (alpha * 0.017453293);    /// from PCL library
}
